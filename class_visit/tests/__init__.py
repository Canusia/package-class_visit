...
(`class_visit.class_visit`) and flat when pip-installed (`class_visit`) — and
`mock.patch` targets and `import_module` arguments are strings, so they cannot
rely on Python's relative-import machinery the way `from ..models import x`
can. Hardcoding either layout breaks the suite on tenants using the other, and
because the breakage is in this file, it takes every module in the package
down with it: see ewu#62.

Import statements in these tests should use relative imports; only dotted-path
*strings* need `PKG`.
"""
...
from ..models import VisitSchedule, VisitReport, VisitReportFile, NotNeededVisit

PKG = (
    'class_visit.class_visit'
...
class UrlResolutionTest(TestCase):
    ...
    def test_instructor_namespace_stub_exists(self):
        ...
        import importlib
        mod = importlib.import_module(f'{PKG}.urls.instructor')
...
class VisitReportFileSerializerTest(TestCase):
    ...
    def test_serializer_fields_match_model(self):
        from ..serializers import VisitReportFileSerializer
...
class ReportFieldsServiceTest(TestCase):
    ...
    def _patch_settings(self, defs=None):
        ...
        return patch(
            f'{PKG}.services.report_fields._get_settings',
    ...
    def test_public_field_names_returns_set(self):
        from ..services.report_fields import public_field_names
        with self._patch_settings():
    ...
    def test_build_report_form_fields_types(self):
        from ..services.report_fields import build_report_form_fields
...
class EmailServiceTest(TestCase):
    ...
    def test_render_template_substitutes_shortcodes(self):
        from ..services.emails import render_template
    ...
    def test_send_app_email_redirects_to_debug_list_in_debug_mode(self, mock_send):
        from ..services.emails import send_app_email
        with self._patch_settings():
    ...
    def test_notify_teacher_visit_scheduled_sends_email(self, mock_vs_objects, mock_send):
        from ..services.emails import notify_teacher_visit_scheduled
        vs = MagicMock()
...
class ConfirmationServiceTest(TestCase):
    ...
    @patch(f'{PKG}.services.confirmation.Site')
    ...
    def test_confirm_visit_returns_none_on_bad_token(self):
        from ..services.confirmation import confirm_visit
...
class PdfServiceTest(TestCase):
    ...
    def test_build_letter_html_returns_string_with_teacher_name(self):
        ...
        with patch(
            f'{PKG}.services.pdf.report_values_for_display',
    ...
    @patch(f'{PKG}.services.pdf.pdfkit')
    def test_visit_letter_pdf_calls_pdfkit_from_string(self, mock_pdfkit):
        from ..services.pdf import visit_letter_pdf
        mock_pdfkit.from_string.return_value = b'%PDF-fake'
        report = self._make_visit_report()
        with patch(f'{PKG}.services.pdf.report_values_for_display',
                   return_value=[]):
